...
client = pymongo.MongoClient("mongodb+srv://Manu:Manu@manu.mudmj2s.mongodb.net/?retryWrites=true&w=majority")  # Replace placeholders
...
def broadcast(update: Update, context: CallbackContext):
    ...
    for collection in [groups_collection, users_collection, channels_collection]:
        ...
        for item in collection.find():
            try:
                context.bot.copy_message(chat_id=item["_id"], from_chat_id=update.effective_chat.id, message_id=message.message_id)
                successful_broadcasts += 1
            except Exception as e:
                failed_broadcasts += 1
                logging.error(f"Failed to broadcast to {collection_type} {item['_id']}: {e}")

        stats_message += f"\n{collection_type.capitalize()}: Successful: {successful_broadcasts}, Failed: {failed_broadcasts}"

    context.bot.send_message(chat_id=admin_user_id[0], text=stats_message)  

    # ... (rest of your broadcast function if needed) 
